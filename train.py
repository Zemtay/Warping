...
def get_model(opt):
    ...
    if opt.dataset == "cifar10" or opt.dataset == "gtsrb":
        #print('*****', opt.device)
        opt.device = 'cpu'  ##defined in the config.py, change there is better and enough
...
def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        num_cross = int(num_bd * opt.cross_ratio)  ##cross validation
        grid_temps = (identity_grid + opt.s * noise_grid / opt.input_height) * opt.grid_rescale
        grid_temps = torch.clamp(grid_temps, -1, 1)   ##choose a medium number
    ...
    # for tensorboard    make the training visible
    if not epoch % 1:
...
def main():
    ...
    opt.data_root = '/Users/angel/learn/Warping-based_Backdoor_Attack-release-main/data/'  ##can be changed in config
    #print('*****', opt.data_root)
    ...
    if opt.continue_training:
        ##print(opt.ckpt_path)
        if os.path.exists(opt.ckpt_path):    ##where to find, really hate to train from scratch
            print("Continue training!!")
            state_dict = torch.load(opt.ckpt_path, map_location=torch.device('cpu'))  ##according to the warning, need to specify the device
            ...
            ##print("check here***")  to show whether this step has been reached
        else:
    ...
    ##print("check here***")
    for epoch in range(epoch_current, opt.n_iters):   ##1000???!!!      when you are continue training, mind the number ???start from 167? 
